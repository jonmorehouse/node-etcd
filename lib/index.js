...
Etcd = (function() {
  ...
  Etcd.prototype.set = function(key, value, options, callback) {
    ...
    opt = this._prepareOpts("keys/" + this._stripSlashPrefix(key), "/v2", this._valueToString(value), options);
    return this.client.put(opt, callback);
  };

  Etcd.prototype.get = function(key, options, callback) {
    var opt, _ref;
    _ref = this._argParser(options, callback), options = _ref[0], this.callback = _ref[1];
    opt = this._prepareOpts("keys/" + this._stripSlashPrefix(key), "/v2", null, options);
    return this.client.get(opt, this._callback);
  };

  Etcd.prototype.create = function(dir, value, options, callback) {
    var opt, _ref;
    _ref = this._argParser(options, callback), options = _ref[0], callback = _ref[1];
    opt = this._prepareOpts("keys/" + this._stripSlashPrefix(dir), "/v2", this._valueToString(value), options);
  ...
  Etcd.prototype._valueToString = function(value) {
  ...
  Etcd.prototype._valueToObject = function(value) {
    var obj;
    try {
  ...
  Etcd.prototype._callback = function(err, res) {
    var node, _i, _len, _ref,
      _this = this;
    if ((res == null) || (err != null) || (res.node == null)) {
      return this.callback(err, res);
    }
    if (res.node.value != null) {
      res.node.value = this._valueToObject(res.node.value);
    }
    if (res.node.nodes != null) {
      _ref = res.node.nodes;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        node = _ref[_i];
        if (node.value != null) {
          (function(node) {
            return node.value = _this._valueToObject(node.value);
          })(node);
        }
      }
    }
